  ...
  return (
    <div className={styles.container} id="contact">
          ...
          <div className={styles.contactDesc}>
            <div className={styles.contactDescLeft}>
              <div className={styles.text1}>
                <p>I would love to hear from you! Whether you have a project in mind, want to collaborate, or simply have a question, feel free to reach out. Here&apos;s how you can get in touch:</p>
              </div>
              ...
              <div className={styles.text2}>
                <p>Let&apos;s connect and discuss how we can work together to
                  bring your ideas to life.Don&apos;t hesitate to drop me a lineor
                  connect through any of the above channels. I am looking
                  forward tohearing from you soon!</p>
              </div>
              <div className={styles.text3}>
                <p>Please note: Due to the high volume of inquiries, response
                  times may vary. However,I make it a priority to respond to
                  all messages in a timely manner.Thank you foryour
                  understanding.</p>
              </div>
            </div>
      ...
      transition={{ delay: 0.3 }} // Add a delay before exit animation
                      ><VscError /> {formErrors.name}</motion.span>
                    )}
                    </AnimatePresence>
                </div>
                <div className={styles.formGroup}>
      ...
      exit={{ opacity: 0, x: -20 }}
      transition={{ delay: 0.3 }} // Add a delay before exit animation
                      ><VscError /> {formErrors.email}</motion.span>
                    )}
                    </AnimatePresence>
                </div>
                <div className={styles.formGroup}>
                  ...
                  <textarea id="message" placeholder='Enter your message here' 
                    className={formErrors.message ? styles.inputError: ""} required 
                    value={form.message || ""}
                    onChange={(e)=> {setField("message", e.target.value), validateField("message",e.target.value)}}
      ...
      exit={{ opacity: 0, x: -20 }}
      transition={{ delay: 0.3 }} // Add a delay before exit animation
                      ><VscError /> {formErrors.message}</motion.span>
                    )}
                  </AnimatePresence>
                </div>
                <div className={styles.formGroup}>
